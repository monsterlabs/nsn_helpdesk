...
class LoadCustomers < ActiveRecord::Migration
  def self.up
    ...
    filename = data_path + '/customers.csv'
    CSV.read(filename).each do |row|
      ...
      Authorization::Maintenance::without_access_control do
        ...
        person_params = { :firstname => row[3].to_s.strip, :lastname => nil, :job_title => row[5].to_s.strip, :notes => row[16].to_s.strip, :company_id => company.id }
        person_params.merge!(:region_id => region.id) unless region.nil?
        @user.person_attributes = person_params

        city_name = row[11].to_s.strip
        city = city_name.empty? ? nil : (City.exists?(:name => city_name, :region_id => region.id) ?  City.find_by_name_and_region_id(city_name, region.id) : City.create!(:name => city_name, :region_id => region.id))
