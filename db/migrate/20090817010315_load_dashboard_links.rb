class LoadDashboardLinks < ActiveRecord::Migration
  def self.up
    if ENV['RAILS_ENV'] != 'test'
      ...
      Dashboard.create!(:label => 'My Tickets', :controller => 'tickets', :action => 'mine', :role_id => @field_manager_role, :parent_id => Dashboard.find_by_role_id_and_controller(@field_manager_role, 'tickets').id)
